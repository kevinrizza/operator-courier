from operatorcourier import courier_api
import argparse
import sys
...
class _CliParser():
    ...
    def __init__(self):
        # Generate the command level parser
...
''')
    ...
    def verify(self):
        ...
        if args.files is None and args.source_dir is None:
            print("Neither source_dir nor files is set. One (and exactly one) of those is required to create a bundle from.")
            parser.print_help()
            exit(1)
        
        courier_api.build_and_verify(files=args.files, source_dir=args.source_dir)
    ...
    def push(self):
        ...
        courier_api.build_verify_and_push(namespace, repository, release, "", files=args.files, source_dir=args.source_dir)